...
std::vector<std::vector<int>> svm_cross_validation(const svm_problem* prob, const svm_parameter* param, int nr_fold, double* target)
{
	std::vector<std::vector<int>> validationIndex;
	int i;
	int* fold_start;
	int l = prob->l;
	int* perm = Malloc(int, l);
	...
	for (i = 0;i < nr_fold;i++)
	{
		...
		else
		{
			...
			for (j = begin;j < end;j++)
			{
				...
				fold.push_back(perm[j]);
			}
			validationIndex.push_back(fold);
		}
		svm_free_and_destroy_model(&submodel);
		free(subprob.x);
		free(subprob.y);
